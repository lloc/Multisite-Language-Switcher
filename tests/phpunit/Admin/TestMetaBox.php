...
final class TestMetaBox extends MslsUnitTestCase {
	...
	private function MetaBoxFactory(): MetaBox {
		...
		return new MetaBox( $options, $collection );
	}
	...
	public function test_render_select_hierarchical(): void {
		...
		Functions\expect( 'get_edit_post_link' )->twice()->andReturn( 'edit-post-link' );
		Functions\expect( 'get_current_blog_id' )->once()->andReturn( 1 );

		$expected = '<ul><li><label for="msls_input_de_DE" class="msls-icon-wrapper flag"><a title="Edit the translation in the de_DE-blog" href="edit-post-link"><span class="flag-icon flag-icon-de">de_DE</span></a>&nbsp;</label><select name="msls_input_region_Code"><option value="0">--some value</option></select><a class="msls-edit-link" href="edit-post-link" target="_blank" title="Edit the translation in the de_DE-blog"><span class="dashicons dashicons-external"></span></a></li></ul>';
		$this->expectOutputString( $expected );
	...
	public function test_render_input( $option, $the_title_times, $current_blog_id_times, $admin_url_times, $edit_post_link_times, $add_query_arg_times, $expected ) {
		...
		Functions\expect( 'msls_content_types' )->once()->andReturn( $post_type );
		Functions\expect( 'msls_options' )->atMost()->times( 1 )->andReturn( $options );
		...
		$this->MetaBoxFactory()->render_input();
	}
