...
namespace lloc\MslsTests\RestApi;
...
final class TestRestApi extends MslsUnitTestCase {
	...
	public function test_prefix_source_language(): void {
		...
		$result = RestApi::prefix_source_language( $post_data, $source_post, 1, 2 );
	...
	public function test_prefix_source_language_is_removable(): void {
		$this->assertTrue(
			method_exists( RestApi::class, 'prefix_source_language' ),
			'prefix_source_language must be a public static method for use with remove_filter'
		);

		$reflection = new \ReflectionMethod( RestApi::class, 'prefix_source_language' );
	...
	public function test_remember_source_blog_writes_user_meta(): void {
		...
		Functions\expect( 'update_user_meta' )
			->once()
			->with( 7, RestApi::LAST_SOURCE_USER_META, 3 );

		RestApi::remember_source_blog( 42, \Mockery::mock( \WP_Post::class ), 3 );
	...
	public function test_get_last_source_blog_id_returns_value(): void {
		...
		Functions\expect( 'get_user_meta' )
			->once()
			->with( 7, RestApi::LAST_SOURCE_USER_META, true )
			->andReturn( '5' );

		$this->assertSame( 5, RestApi::get_last_source_blog_id() );
	}
