jQuery( document ).ready(
	function ( $ ) {
		...
		// Inject the "Add from Translation" page-title-action next to the
		// core "Add New" anchor. WP exposes no server-side hook here, so
		// this is the standard pattern used by Jetpack, Yoast, etc.
		...
		var href = '#TB_inline?width=620&height=480&inlineId=' + encodeURIComponent( config.inlineId );
		var $button = $( '<a>' )
		...
		function renderItems( items ) {
			...
			$.each(
				...
				function ( _, item ) {
					...
					if ( item.date_gmt ) {
						try {
							date = new Date( item.date_gmt ).toLocaleDateString();
						} catch ( e ) {
							date = item.date_gmt;
						}
					}

					var $meta = $( '<span>' ).addClass( 'msls-tp-item-meta' ).text( date );
					var $title = $( '<span>' ).addClass( 'msls-tp-item-title' ).text( item.title );

					var $item = $( '<li>' )
						.addClass( 'msls-tp-item' )
						.attr( 'role', 'button' )
						.attr( 'tabindex', '0' )
		...
		function loadList( sourceBlogId ) {
			$list.empty();
			$status.text( config.i18n.loading );

			wp.apiFetch(
				{
					path: '/msls/v1/untranslated-posts?' + $.param(
						{
		...
		function createTranslation( sourcePostId, sourceBlogId ) {
		...
		$blogSelect.on(
			'change',
			function () {
				var sourceBlogId = parseInt( $( this ).val(), 10 );
				if ( ! sourceBlogId ) {
					$list.empty();
					$status.text( '' );
					return;
		...
		$list.on(
			...
			function ( event ) {
				...
				var sourcePostId = parseInt( $( this ).data( 'source-post-id' ), 10 );

				if ( ! sourceBlogId || ! sourcePostId ) {
					return;
				}

				createTranslation( sourcePostId, sourceBlogId );
			}
		);
	}
);
